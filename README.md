...
- **Selecting text and editing highlights with the pen:** the pen always starts a stroke, and moving before the long-press interval (KOReader's `Long-press interval`, 500 ms by default) draws as usual. Hold the pen still for that interval and the stroke is dropped and the plugin acts like a finger long-press. On text that is not highlighted, text selection starts: keep the pen down and drag to extend the selection to a sentence or more, then lift it to get the normal highlight menu (or the highlight is saved directly, depending on your KOReader highlight settings). On an existing highlight, its edit menu opens instead. Holding on an existing stroke opens the stroke menu first. Switch this off with `Hold pen still to select text or edit highlight` in the plugin menu.
