...
- **Selecting text with the pen:** hold the pen still on text for the long-press interval (KOReader's `Long-press interval`, 500 ms by default) and the stroke is dropped and text selection starts. Keep the pen down and drag to extend the selection to a sentence or more, then lift it: you get the normal highlight menu (or the highlight is saved directly, depending on your KOReader highlight settings). Moving the pen before the interval ends draws as usual. Holding on an existing stroke opens the stroke menu instead. Switch this off with `Hold pen still to select text` in the plugin menu.
- **Highlights:** putting the pen down on an existing highlight opens its menu immediately, like a finger tap, with no waiting and no stroke drawn. Switch this off with `Pen tap on a highlight opens its menu` in the plugin menu if you want to draw over highlighted text.
